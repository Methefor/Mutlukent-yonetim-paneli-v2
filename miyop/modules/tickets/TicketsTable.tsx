...
type Row = { id: string; created_at: string; category: string; title: string; status: string; sube_id: string | null };
...
export function TicketsTable() {
  ...
  useEffect(() => {
    const load = async () => {
      let query = supabase
        .from("tickets")
        .select("id, created_at, category, title, status, sube_id")
  ...
  const filtered = rows.filter((r) =>
    [new Date(r.created_at).toLocaleString(), r.category, r.title, r.status, r.sube_id ?? ""].some((v) =>
