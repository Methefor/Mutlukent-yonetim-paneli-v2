...
type Row = { id: string; created_at: string; staff_name: string; movement: string; sube_id: string | null };
...
export function HRTable() {
  ...
  useEffect(() => {
    const load = async () => {
      let query = supabase
        .from("hr_records")
        .select("id, created_at, staff_name, movement, sube_id")
  ...
  const filtered = rows.filter((r) =>
    [new Date(r.created_at).toLocaleString(), r.staff_name, r.movement, r.sube_id ?? ""].some((v) =>
